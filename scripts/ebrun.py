...
def run_eb(table, tumor_bam, output, pon_list, chrom, log, threads, EBparams, full_output,
    ...
    # but can be easily included in the csv2bed script
    ...
    # cut -f $({pon2cols}< {sample_list}) creates a cut command only including the desired cols for computation
    pipe_cmd = f"{pileup_cmd} | cut -f $({pon2cols} < {sample_list}) | {cleanpileup} | {pile2count} > {matrix_file}"
    # do the pileup to matrix_file
    run_cmd(pipe_cmd, multi=False)
    # cleanup
    ...
    # get the pon_matrix containing the Pon coverages in Alt and Ref from the mutmatrix file
...
def run_eb_from_cache(table, tumor_bam, output, pon_list, chrom, log, threads, EBparams, full_output,
                      AB_cache_file, matrix_cache_file,
                      cleanpileup,
                      csv2bed,
                      pile2count,
                      matrix2EBinput,
    ...
    if mut_df.empty:
        ...
        # if sample_in_pon == 0, then sample is not in PoN
        ...
        # ########################################### CACHE FROM ABcache ###########################
            ...
            tumor_matrix_file = target_pileup_from_mut(
                table, base_file, tumor_bam, chrom)
            # check if matrix_file has input
            ...
            show_output(
                f"Pileup matrix for for chrom {chrom} of {tumor_bam} merged with AB matrix. Going on with EB computation...",
                color='normal'
            )
