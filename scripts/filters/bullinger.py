...
parser = argparse.ArgumentParser('filters annovar output with bullinger params')
parser.add_argument('-TM2_limit', type=int, help='test value 1', default=4)
...
TM2_limit, i, o = args.TM2_limit, args.input, args.output
...
def bullinger_filter(data, output):
    '''
    creates filtered output using the bullinger filtering
    input: pd.dataframe of unfiltered annovar output
    output: 
        - filtered/sample_tumor-normal_bull.csv
        - filtered/sample_tumor-normal_bull_somatic.csv
        - filtered/sample_tumor-normal_bull_germline.csv
        - filtered/sample_tumor-normal_bull_loh.csv
    '''
...
bullinger_filter(df, output_base)
