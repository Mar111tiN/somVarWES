...
def filter2(df, stringency="", thresh={}, config={}):
    ...
    return sort_df(filter2_df, cols=sort_cols), sort_df(dropped_candidates_df, cols=sort_cols), list_len


def get_filter2(mut_file, filter2_output,
                filterbam_output=None, config={}):
    ...
    if "xls" in os.path.splitext(filter_file)[1]:
        filter_settings = pd.read_excel(
            filter_file, sheet_name=config["filter_sheet"], index_col=0)[:4]
    else:
    ...
    for stringency in ['loose', 'moderate', 'strict']:
        filter2_dfs[stringency], dropped_dfs[stringency], df_lengths[stringency] = filter2(
            filter1_df, stringency=stringency, thresh=filter_settings.loc[f"filter2-{stringency}"], config=config)
