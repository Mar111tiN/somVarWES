...
def main(s):
    ...
    # ############## COSMIC70 ##############
    cosmic70_dict = {"haematopoietic_and_lymphoid_tissue": 5, "bone": 2}
    ...
    def get_PoN_info(df):
        ...
        df.loc[:, "PoN-Alt-NonZeros"] = df["PoN-Alt"].str.count(r"[0-9]+") - df[
            "PoN-Alt"
        ].str.count("0")
    ...
    def resort_cols(df):
        ...
        pop_cols = [
            col
            for col in cols
            if re.match("|".join(["avsnp", "dbSNP", "esp6500", "1000g", "gnomAD"]), col)
        ]
    ...
    def get_clinical_scores(df):
        ...
        def cosmic91_score(row):
            return (
                (
                    1
                    + cosmic91_type_score.get(row["types"], 0)
                    + cosmic91_location_score.get(row["location"], 0)
                )
                * (row["types"] != ".")
                * int(row["count"])
            )
        ...
        df["cosmic91_score"] = (
            df["cosmic91_type"]
            .str.extractall(cosmic91_pattern)
            .apply(cosmic91_score, axis=1)
            .reset_index()
            .drop(columns="match")
            .groupby("level_0")
            .sum()
        ...
        for col in clinscore_cols:
            print("            ", col)
            df.loc[df[col] != ".", "ClinScore"] += (
                ClinScore[col] * df[df[col] != "."][col]
            )
        return df
    ...
    def get_gene_lists(df, candidate_list, driver_list, hotspot_list):
        ...
        if hotspot_list:
            ...
            df = df.merge(
                hotspots, how="left", on=["Chr", "Start", "Ref", "Alt", "Gene"]
            )
