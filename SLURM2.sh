...
snakemake --unlock --rerun-incomplete -s Snakefile2;
snakemake -s Snakefile2 --dag | awk '$0 ~ "digraph" {p=1} p' | dot -Tsvg > dax/dag.svg;
snakemake -s Snakefile2 --use-conda --rerun-incomplete --cluster-config configs/cluster/somvar-cluster.json --drmaa "$DRMAA" -prk -j 1000;
